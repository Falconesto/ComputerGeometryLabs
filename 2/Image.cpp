...
//    if ((line->thickness <= 1)) {
        ...
        if (line->start.x == line->end.x) {
            ...
            line->start.y = line->start.y - 0.5;
            line->end.y = line->end.y - 0.5;
            std::vector<Point> rectPoints = getDots(line, line->thickness / 2, 0);
            std::swap(rectPoints[0], rectPoints[1]);
            std::swap(rectPoints[3], rectPoints[1]);
            std::swap(rectPoints[3], rectPoints[2]);
//            drawStraightThinLineY(line, line->brightness * line->thickness);
            if(line->thickness>1){
                draw45Line(rectPoints, line->brightness,1, 2);
            }else{
                draw45Line(rectPoints, line->brightness, line->thickness, 2);
            }
        } else {
            if (line->start.y == line->end.y) {
                line->start.x = line->start.x - 0.5;
...
//                drawStraightThinLineX(line, line->brightness * line->thickness);
                std::vector<Point> rectPoints = getDots(line, 0, line->thickness / 2);
                if(line->thickness>1){
                    draw45Line(rectPoints, line->brightness,1, 2);
                }else{
                    draw45Line(rectPoints, line->brightness, line->thickness, 2);
...
//                drawWuLine(line->start, line->end, line->thickness, line->brightness);
                if(line->thickness>1){
                    draw45Line(rectPoints, line->brightness,1, 0);
                }else{
                    draw45Line(rectPoints, line->brightness, line->thickness, 1);
                }
...
void Image::draw45Line(std::vector<Point> points, int color, double brightness, int bad) {
    double alfa = 0;
    for (int i = round(std::min(std::min(points[0].y, points[1].y), std::min(points[2].y, points[3].y)));
        ...
        for (int j = round(std::min(std::min(points[0].x, points[1].x), std::min(points[2].x, points[3].x)));
            ...
            if (isInside45(points, j, i)) {
                ...
                for (int k = 1; k < 5; k++) {
                    if (isInside(points, j + (k * 0.125)-alfa, i + (k * 0.125)-alfa)) {
                        count++;
                    }
                    if (isInside(points, j - (k * 0.125)+alfa, i + (k * 0.125)-alfa)) {
                        count++;
                ...
                if(count>bad) {
                    putPoint(j, i, pow(((double) count / (double) 17)*brightness, 1 / 1), color);
                }
            }
        }
    }
...
bool Image::isInside(std::vector<Point> points, double checkX, double checkY) { // 0-правее, 1- левее
...
bool Image::isInsideXtoTheLeft(Point first, Point second, double checkX, double checkY) { // 0-правее, 1- левее
...
bool Image::isInsideXtoTheRight(Point first, Point second, double checkX, double checkY) { // 0-левее, 1- правее
    return ((((checkY - first.y) * (second.x - first.x) / (second.y - first.y)) + first.x) <= (checkX) + 0.1) ||
           ((((checkY - first.y) * (second.x - first.x) / (second.y - first.y)) + first.x) <= (checkX) - 0.1);
}

bool Image::isInsideYtoTheUp(Point first, Point second, double checkX, double checkY) { // 0-ниже, 1- выше
...
bool Image::isInsideYtoTheDown(Point first, Point second, double checkX, double checkY) { // 0-выше, 1- ниже
    return ((((second.y - first.y) * (checkX - first.x) / (second.x - first.x)) + first.y) <= (checkY) + 0.1) ||
           ((((second.y - first.y) * (checkX - first.x) / (second.x - first.x)) + first.y) <= (checkY) - 0.1);
}
